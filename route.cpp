...
Route::StopsContainer::const_iterator Route::begin() const {return stops_.begin();}
Route::StopsContainer::const_iterator Route::end() const {return stops_.end();}
...
ostream& operator<<(ostream& out, const Route& r) {
    ...
    return out<<r.Stops()<<" stops on route, "<<r.UniqueStops()<<" unique stops, "
    		<<int(distance)<<" route length, "<< distance / r.LineDistance()<<" curvature";
}
...
string_view CircleRoute::Delimiter() const {return CircleRoute::delimiter;;}

int CircleRoute::Stops() const {return route_.size();}

string_view TwoWayRoute::Delimiter() const {return TwoWayRoute::delimiter;}

int TwoWayRoute::Stops() const {return route_.size() * 2 - 1;}


DistanceType TwoWayRoute::Distance() const {
	auto distance = [this](const string& from, const string& to) {return this->db_->Distance(from, to);};
